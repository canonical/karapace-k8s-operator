...
def test_build_and_deploy(juju: JujuFixture, karapace_charm, app_charm):
    gather(
        ...
        juju.ext.model.deploy(app_charm, application_name=DUMMY_NAME, num_units=1),
    )
    ...
    juju.ext.model.wait_for_idle(apps=[KAFKA, APP_NAME, DUMMY_NAME], idle_period=30, timeout=1800)
